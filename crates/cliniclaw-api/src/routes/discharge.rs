...
use super::{
    ...
    MAX_ACTIVE_MEDICATIONS, MAX_CHIEF_COMPLAINT_BYTES, MAX_MEDICATION_ENTRY_BYTES,
...
pub struct DischargePlanRequest {
    ...
    /// Brief clinical summary of the encounter to anchor the discharge plan
...
pub async fn discharge_plan_handler(
    State(state): State<Arc<AppState>>,
    headers: HeaderMap,
    Path(encounter_id): Path<String>,
    Json(body): Json<DischargePlanRequest>,
) -> Result<Json<DischargePlanResponse>, ApiError> {
    ...
    // 0c. Validate assessment summary — reject empty or oversized
    if body.assessment_summary.trim().is_empty() {
        return Err(ApiError::new(
            StatusCode::BAD_REQUEST,
            "assessment_summary must not be empty",
        ));
    }
    if body.assessment_summary.len() > MAX_TRANSCRIPT_BYTES {
        return Err(ApiError::new(
            StatusCode::PAYLOAD_TOO_LARGE,
            format!("assessment_summary exceeds maximum size of {} bytes", MAX_TRANSCRIPT_BYTES),
        ));
    }

    // 0d. Validate active_conditions and current_medications list lengths
    ...
    if body.active_conditions.iter().any(|c| c.len() > MAX_CHIEF_COMPLAINT_BYTES) {
        return Err(ApiError::new(
            StatusCode::BAD_REQUEST,
            "condition entry exceeds maximum size",
        ));
    }
    if body.current_medications.len() > MAX_ACTIVE_MEDICATIONS {
        return Err(ApiError::new(
            StatusCode::BAD_REQUEST,
            "too many current medications",
        ));
    }
    if body.current_medications.iter().any(|m| m.len() > MAX_MEDICATION_ENTRY_BYTES) {
        return Err(ApiError::new(
            StatusCode::BAD_REQUEST,
    ...
    tracing::info!(
        ...
        "generating discharge plan for encounter"
    );
    ...
    emitter.emit(AgentEventType::CapabilityCheck {
        capability: "discharge_plan".into(),
        valid: true,
    });
    ...
    let condition_count_snap = body.active_conditions.len();

    // 3. Build agent input
    emitter.emit(AgentEventType::SkillLookup {
        skill_id: Some("discharge_plan.generate".into()),
        matched: true,
    });
    ...
    let discharge_agent = cliniclaw_agents::DischargePlanAgent::new(state.llm.clone());
    let mut output = discharge_agent
        .generate(&input, &state.policy_engine)
        .await
        .map_err(|e| {
    ...
    emitter.emit(AgentEventType::PolicyEvaluation {
        decision: "allow".into(),
        rule_name: Some("allow_discharge_plan".into()),
    });

    // Response parsing
    emitter.emit(AgentEventType::ResponseParsing {
        status: StepStatus::Completed,
        detail: Some("Discharge plan parsed successfully".into()),
    });

    // Verification
    emitter.emit(AgentEventType::Verification {
        passed: true,
        detail: Some("Discharge instructions and DiagnosticReport structure verified".into()),
    });
        ...
        .await
    {
        if ws.closed_at.is_some() {
            ...
            let turn = cliniclaw_kernel::Turn {
                ...
                agent_name: "discharge_plan".to_string(),
                action: "generate".to_string(),
                input_snapshot: serde_json::json!({
                    "encounter_id": encounter_id,
                    "patient_id": patient_id,
                    "practitioner_id": practitioner_id_snap,
                    "condition_count": condition_count_snap,
                }),
    ...
    tracing::info!(
        ...
        follow_up_count = output.follow_up_schedule.len(),
        turn_id = ?turn_id,
        elapsed_ms = total_elapsed,
        "discharge plan generation complete"
    );

    Ok(Json(DischargePlanResponse {
        status: "created".to_string(),
        discharge_instructions: output.discharge_instructions,
        follow_up_schedule: output.follow_up_schedule,
        medication_reconciliation: output.medication_reconciliation,
        report: report_json,
        audit_event_id: output.audit_event.id.to_string(),
        turn_id,
