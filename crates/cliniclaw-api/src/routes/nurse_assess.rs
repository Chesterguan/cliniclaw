...
use super::{
    ...
    MAX_CHIEF_COMPLAINT_BYTES, MAX_TRANSCRIPT_BYTES,
};

#[derive(Debug, serde::Deserialize)]
pub struct NurseAssessRequest {
    pub practitioner_id: String,
    /// Type of assessment: "admission" for initial, "ongoing" for subsequent
...
pub struct NurseAssessResponse {
    pub status: String,
    pub nursing_assessment: String,
    pub fall_risk_score: u8,
    pub pain_score: u8,
    pub braden_score: u8,
    /// FHIR Observation resource created for the nursing assessment
...
pub async fn nurse_assess_handler(
    State(state): State<Arc<AppState>>,
    headers: HeaderMap,
    Path(encounter_id): Path<String>,
    Json(body): Json<NurseAssessRequest>,
) -> Result<Json<NurseAssessResponse>, ApiError> {
    ...
    // 0c. Validate assessment_type
    if body.assessment_type.trim().is_empty() {
        return Err(ApiError::new(
            StatusCode::BAD_REQUEST,
            "assessment_type must not be empty",
        ));
    }
    if body.assessment_type.len() > MAX_CHIEF_COMPLAINT_BYTES {
        return Err(ApiError::new(
            StatusCode::BAD_REQUEST,
            "assessment_type exceeds maximum size",
        ));
    }

    // 0d. Validate vitals text — reject empty or oversized
    if body.vitals_text.trim().is_empty() {
        return Err(ApiError::new(
            StatusCode::BAD_REQUEST,
            "vitals_text must not be empty",
        ));
    }
    if body.vitals_text.len() > MAX_TRANSCRIPT_BYTES {
        return Err(ApiError::new(
            StatusCode::PAYLOAD_TOO_LARGE,
            format!("vitals_text exceeds maximum size of {} bytes", MAX_TRANSCRIPT_BYTES),
        ));
    }
    ...
    tracing::info!(
        ...
        "running nursing assessment for encounter"
    );
    ...
    // Role check — nursing assessments are typically performed by nurses
    let role = body.practitioner_role.clone().unwrap_or_else(|| "nurse".to_string());
    ...
    let assessment_type_snap = body.assessment_type.clone();

    // 3. Build agent input
    emitter.emit(AgentEventType::SkillLookup {
        skill_id: Some("nurse_assess.evaluate".into()),
        matched: true,
    });
    ...
    let nurse_agent = cliniclaw_agents::NurseAssessAgent::new(state.llm.clone());
    let mut output = nurse_agent
    ...
    emitter.emit(AgentEventType::PolicyEvaluation {
        decision: "allow".into(),
        rule_name: Some("allow_nurse_assess".into()),
    });

    // Response parsing
    emitter.emit(AgentEventType::ResponseParsing {
        status: StepStatus::Completed,
        detail: Some("Nursing assessment parsed successfully".into()),
    });

    // Verification
    emitter.emit(AgentEventType::Verification {
        passed: true,
        detail: Some("Risk scores and Observation structure verified".into()),
    });
    ...
    // 6. Write Observation to FHIR via backend trait
    let created_obs: cliniclaw_fhir::Observation =
        cliniclaw_fhir::backend::create_typed(state.fhir.as_ref(), &output.observation)
            .await
            .map_err(ApiError::from)?;

    let observation_json = serde_json::to_value(&created_obs)?;

    emitter.emit(AgentEventType::FhirWrite {
        resource_type: "Observation".into(),
        resource_id: created_obs.id.clone(),
    });
        ...
        .await
    {
        if ws.closed_at.is_some() {
            ...
            let turn = cliniclaw_kernel::Turn {
                ...
                agent_name: "nurse_assess".to_string(),
                action: "evaluate".to_string(),
                input_snapshot: serde_json::json!({
                    "encounter_id": encounter_id,
                    "patient_id": patient_id,
                    "practitioner_id": practitioner_id_snap,
                    "assessment_type": assessment_type_snap,
                }),
                output_snapshot: observation_json.clone(),
    ...
    tracing::info!(
        ...
        pain_score = output.pain_score,
        braden_score = output.braden_score,
        turn_id = ?turn_id,
        elapsed_ms = total_elapsed,
        "nursing assessment complete"
    );

    Ok(Json(NurseAssessResponse {
        status: "created".to_string(),
        nursing_assessment: output.nursing_assessment,
        fall_risk_score: output.fall_risk_score,
        pain_score: output.pain_score,
        braden_score: output.braden_score,
