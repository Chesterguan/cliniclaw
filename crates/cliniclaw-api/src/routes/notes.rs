...
pub async fn generate_note(
    ...
    // 0d. Validate optional fields that flow into Claude prompt
    if let Some(ref cc) = body.chief_complaint {
        if cc.len() > MAX_CHIEF_COMPLAINT_BYTES {
            return Err(ApiError::new(
                StatusCode::BAD_REQUEST,
    ...
    emitter.emit(AgentEventType::CapabilityCheck {
        capability: "note_generation".into(),
        valid: true,
    });
    ...
    let transcript_len_snap = body.transcript.len();

    // 3. Build agent input with skill-aware context
    emitter.emit(AgentEventType::SkillLookup {
        skill_id: Some("ambient_doc.generate_note".into()),
        matched: true,
    });
    ...
    let mut output = state
        .ambient_agent
        .generate_note(&input, &state.policy_engine)
        .await
        .map_err(|e| {
    ...
    emitter.emit(AgentEventType::PolicyEvaluation {
        decision: "allow".into(),
        rule_name: Some("allow_note_generation".into()),
    });

    // Response parsing
    emitter.emit(AgentEventType::ResponseParsing {
        status: StepStatus::Completed,
        detail: Some("SOAP note parsed successfully".into()),
    });
        ...
        .await
    {
        ...
        let turn = cliniclaw_kernel::Turn {
            ...
            input_snapshot: serde_json::json!({
                ...
                "transcript_len": transcript_len_snap,
            }),
        ...
        // Check for chain triggers (agent-to-agent collaboration)
        super::chain::check_chain_triggers(
            &state,
            &tid,
            "ambient_doc",
            &encounter_id,
