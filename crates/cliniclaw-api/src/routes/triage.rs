...
pub struct TriageResponse {
    ...
    /// FHIR Observation resource created for the triage result
...
pub async fn triage_handler(
    ...
    // 0c. Validate chief complaint — reject empty or oversized
    if body.chief_complaint.trim().is_empty() {
        return Err(ApiError::new(
            StatusCode::BAD_REQUEST,
            "chief_complaint must not be empty",
        ));
    }
    if body.chief_complaint.len() > MAX_CHIEF_COMPLAINT_BYTES {
        return Err(ApiError::new(
            StatusCode::BAD_REQUEST,
            "chief complaint exceeds maximum size",
        ));
    }
    ...
    tracing::info!(
        ...
        "running triage assessment for encounter"
    );
    ...
    let chief_complaint_snap = body.chief_complaint.clone();

    // 3. Build agent input
    emitter.emit(AgentEventType::SkillLookup {
        skill_id: Some("triage_assess.evaluate".into()),
        matched: true,
    });
    ...
    let triage_agent = cliniclaw_agents::TriageAssessAgent::new(state.llm.clone());
    let mut output = triage_agent
    ...
    emitter.emit(AgentEventType::PolicyEvaluation {
        decision: "allow".into(),
        rule_name: Some("allow_triage_assess".into()),
    });

    // Response parsing
    emitter.emit(AgentEventType::ResponseParsing {
        status: StepStatus::Completed,
        detail: Some("Triage assessment parsed successfully".into()),
    });

    // Verification
    emitter.emit(AgentEventType::Verification {
        passed: true,
        detail: Some("ESI level and Observation structure verified".into()),
    });
    ...
    Ok(Json(TriageResponse {
        ...
        triage_level: output.triage_level,
        acuity_label: output.acuity_label,
